...
export class InteractiveMapComponent implements OnChanges, AfterViewInit {
  ...
  ngOnChanges() {
    let color = "red";
    if (this.me) {
      if (this.me.gender == 1) color = "red";
      else this.me.gender == 2;
      color = "blue";
      let newCenter = new google.maps.LatLng(
        parseFloat(this.me.latitude),
        parseFloat(this.me.longitude)
      );
  ...
  pushCarres() {
    ...
    if (this.users) {
      for (var i = 0; i < this.users.length; i++) {
        ...
        let marker = new google.maps.Marker({
          position: {
            lat: parseFloat(this.users[i].latitude),
            lng: parseFloat(this.users[i].longitude),
          },
          icon: {
            url: userColor,
          },
